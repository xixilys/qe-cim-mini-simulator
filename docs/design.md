...
在引入 Uchino 等 2025 的复数 Ozaki-II 方案之后，当前系统不应再只被理解为“一个近似的复数 4M/3M 原型”，而应明确区分为两类模式：

#### 模式 A：近似子空间模式

这条模式对应当前已经完成的 SystemC 原型：

- 复数 4M / 3M 拆分
- `BF16` / `FP32` / `INT8_EMU`
- 目标是验证子空间 block MVM、`H_sub/S_sub` 构造、Hermitian 化与误差门控
...
#### 模式 B：完整 FP64 复数 GEMM emulation 模式

这条模式对应 Uchino 等论文给出的复数 Ozaki-II 路线：

- `FP64 complex input`
- `缩放/截断 -> CRT residue -> Karatsuba 模乘 -> CRT 重构 -> 反缩放`
- 目标是实现 `ZGEMM` 级别的复数矩阵乘法

它适合：

- 需要完整 `FP64` 复数输出的主路径
- 不能接受 `INT8_EMU/BF16/FP32` 近似误差的算子
- 作为未来 QE 复数 GEMM 后端的精确模式
...
- 本节的 `3M` 指的是**低精度近似 block MVM 的 Gauss/Karatsuba 变体**
- 它会放大量化误差，因此不能默认开启

这与后文完整 FP64 Ozaki-II 路线中的 `Karatsuba` 不是同一层概念。  
在 CRT 模式里，Karatsuba 作用在**模域整数矩阵**上，每个模乘本身是精确的，不属于这里这种近似 3M。
...
### 第三步：把完整 FP64 Ozaki-II 路线并入系统级闭环

- 将完整 `FP64 complex GEMM emulation` 接到当前复数阵列调度模型
- 明确 residue buffer、CRT 重构、Karatsuba 3M 的时延模型
- 在需要高精度的路径上优先选择完整模式

### 第四步：再继续细化底层 primitive
...
- 以实数乘法器为底层 primitive
- 在近存逻辑中完成复数 4M / 3M 编排
- 进一步基于 Uchino 2025 补齐完整的 `FP64 complex GEMM emulation` 路线
