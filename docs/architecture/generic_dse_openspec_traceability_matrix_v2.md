...
| `dft-qe-workload-adapter` | §2 System context; §4.1 WorkloadPackage domain metadata; §5.3 seed mappings; §9.1 final report sections | Keeps QE/DFT as a reference workload adapter while preserving generic IR and architecture contracts. |
...
| Workload and IR contracts | Versioned four-level IR stack; ComputeGraph is operator-agnostic and graph-shape neutral; graph lowering; TensorSpec defines units; adapter-declared workflow coverage; DFT/QE adapter maps SCF workflow | §2.1, §3.1, §4.1, §4.1.1, §4.1.3, §4.1.4, §4.4, §8.2 |
...
| Step3 simulation/evidence | Step3 consumes persisted Step2 handoff artifacts; Step3 validates before simulation; Step3 reconstructs replayable request; Step3 emits full-flow evidence and structured status; Step3 cross-step validation covers generic and adapter workflows | §4.1.6, §8.1–§8.4, §9.3–§9.4 |
...
| DFT/QE specialization | Canonical QE SCF graph; DFT metrics; calibration; algorithm-freeze priorities; DFT regression suite | §2.1, §2.2, §5.3, §8.2, §9.1, §9.3 |

## Review Checklist

- The professional architecture document defines the full end-to-end DSE run before subsystem details.
- Workload-family workflows define accepted sources, graph/lowering policy, mapping seeds, simulation coverage, adapter-domain validation, and claim boundary for each supported non-QE and QE family.
...
- Cross-step tests must exercise Step1→Step2→Step3 continuity for representative non-QE workloads and the DFT/QE adapter, not only Step3-local fabricated inputs.
