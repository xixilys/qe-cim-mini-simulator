...
|---|---|---|---|
| Campaign control plane | `campaign.json` | Control plane | All stages, final audit |
| Campaign ledger | `campaign_ledger.json` | Control plane | All stages, final audit |
| Campaign evaluation plan | `campaign_evaluation_plan.json` | Control plane | Step3, Step4, final audit |
| Campaign search admission plan | `campaign_search_admission_plan.json` | Control plane | Step2 materialization, Step3 admission audit, ledger |
...
| Search checkpoint | `search_checkpoint.json` | Step2 | Search replay, feedback audit |
| Top-K candidate queue provenance | `top_k_candidate_queue.json` | Step2 | Candidate ordering audit; not Step3 admission |
...
| Trial state ledger | `trial_state_ledger.json` | Step2 | Step3 admission, audit, search feedback |
...
| Search iteration plan | `search_iteration_plan.json` | Step4 feedback bridge | Next Step2/Campaign scheduling; not Step3 admission |
...
| DFT hardware evidence matrix | `dft_hardware_evidence_matrix.json` plus validation/status files | DFT profile helper | Candidate × major-kernel trust matrix derived from either explicit kernel dispositions or the release-gate rollup; feeds ledger/workplan/reporting without upgrading deliverable completion |
| DFT hardware PPA ranking | `dft_hardware_ppa_ranking.json` plus Pareto/validation/status files | DFT profile helper | Hardware-only candidate-stamped PPA ranking, Step5 scoped ranking/Pareto visibility, DFT goal audit evidence |
| DFT candidate-specific PPA provenance | `dft_candidate_specific_ppa_provenance_audit.json`, `dft_hardware_tie_breaker_execution_queue.json`, plus validation/status files | DFT profile helper | Verifies parsed PPA rows have fresh candidate-specific command/tool provenance; queues fresh golden/sim/synth/Vivado/DC reruns when evidence was copied/wrapped or smoke-scoped |
| DFT candidate-specific PPA execution | `dft_candidate_specific_ppa_execution.json` plus validation/status files and per-unit `fresh_tool_work/` evidence | DFT profile helper | Executes selected tie-breaker queue slices into canonical candidate-specific evidence, records IC/EDA tool/version and command provenance, and keeps parser/release/winner gates fail-closed |
| DFT architecture winner resolution | `dft_architecture_winner_resolution.json` plus validation/status files | DFT profile helper | Fail-closed FPGA/ASIC best-architecture gate; rejects tied hardware PPA and provenance-weak PPA rows, and records exact candidate-specific evidence needed before winner claims |
...
`search_checkpoint.json`, `top_k_candidate_queue.json`,
`architecture_candidate_generation_report.json`,
`architecture_screening_report.json`, `trial_state_ledger.json`,
`mapping_candidates.jsonl`, `screening_results.jsonl`, and
`promotion_decisions.jsonl`.  The search checkpoint, Top-K provenance queue,
...
budget, claim boundary, Step1/Step2/selected-Step3 references, and optional
registry lifecycle linkage.  These artifacts remain bounded pilot provenance;
they do not widen evidence fanout or upgrade the campaign to deliverable
completion on their own.

Campaign-manager planning note (2026-05-21): the same pilot also writes
`campaign_evaluation_plan.json`, a budgeted Step3 handoff that admits only the
selected-entry queue rows and keeps Top-K rows as deferred provenance.  This
plan is the first-class bridge between search provenance and executable
campaign scheduling; it still does not authorize broad evidence runs or
release-complete claims on its own.

Search-feedback closure note (2026-05-21): the campaign plan now records an
explicit `admission_control` block.  The default remains
`selected_entry_only`; `top_k_candidate_queue.json` cannot become executable
work merely because it is ranked.  Even when a future campaign budget requests
Top-K widening, execution stays fail-closed until those candidates are
materialized into `step3_simulation_queue.json`.  Step4
`feedback_update.json` and `calibration_record.json` also carry generic
candidate/metric observations that can be routed to
`SearchPolicy.observe(candidate_id, metrics)` through the Step2
`search_checkpoint.json` alias map.  The bridge tries the full ordered
candidate-ref set (`search_policy_candidate_id`, runtime `candidate_id`,
`mapping_candidate_id`, `mapping_parameter_hash`, and other stable refs) and
applies feedback only when one of those aliases resolves to a persisted search
candidate.  Full-flow pilots now emit `search_iteration_plan.json`, which
rehydrates the persisted search problem, applies Step4 feedback, and records the
next proposal ordering.  This artifact is still proposal provenance only:
feedback may update search checkpoint state and later proposal ordering, but it
does not bypass the Campaign/Step3 admission gate.

Campaign search-admission note (2026-05-21): full-flow pilots now also emit
`campaign_search_admission_plan.json` after `search_iteration_plan.json`.  This
is the Campaign Manager fail-closed bridge from feedback-informed proposal
ordering to future scheduling: it may request Step2 materialization under an
explicit widened budget, but it does not run evidence and is not itself a Step3
queue.  `step2/step3_simulation_queue.json` remains the only Step3 admission
authority, and the search-admission plan keeps `execution_allowed=false`,
`hidden_evidence_fanout_allowed=false`, `trusted_final_claim=false`, and
...
green.  `top_k_candidate_queue.json` is deliberately separate: it records
search ordering and future execution-order suggestions with
`queue_mode=top-k-provenance-only`, but it is not consumed as the Step3
admission queue and cannot schedule candidates by itself.

`trial_state_ledger.json` is the generic Step2 lifecycle rollup over
`architecture_candidate_set.json`, `search_checkpoint.json`,
`top_k_candidate_queue.json`, `mapping_candidates.jsonl`,
`screening_results.jsonl`, `promotion_decisions.jsonl`, and
`step3_simulation_queue.json`.  Each row carries `campaign_id`,
`workload_run_id`, `trial_id`, candidate identity, `parameter_hash`, legal
state (`generated`, screened/promoted flags, queue state), blockers,
transition history, and artifact refs.  It is still Step2 provenance: it makes
search and Step3 admission auditable, but it cannot replace Step3 simulation,
Step4 adjudication, or Step5 claim reporting.
...
DFT trial-state ledgers are profile-owned extensions/citations layered on top
of the generic `trial_state_ledger.json` contract.  The DFT helper materializes
its profile-specific evidence view as `dft_trial_state_ledger.json`,
