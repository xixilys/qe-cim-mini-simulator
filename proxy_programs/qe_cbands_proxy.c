...
int main(void) {
    ...
    desc.opcode = QEBS_OPCODE_ADAPTER_DEFINED;
    desc.n_bands = (uint32_t)env_int("QEBS_QE_NBND", (int)desc.n_bands);
    desc.n_basis = (uint32_t)env_int("QEBS_QE_NPWX", (int)desc.n_basis);
    desc.n_kpoints = (uint32_t)env_int("QEBS_QE_NKS", (int)desc.n_kpoints);
    desc.n_spin = (uint32_t)env_int("QEBS_QE_NSPIN", (int)desc.n_spin);
    desc.max_iterations =
        (uint32_t)env_int("QEBS_QE_MAX_SCF_ITERS", (int)desc.max_iterations);
    desc.control_policy =
    ...
    if (rc != 0) {
        fprintf(stderr, "qe cbands proxy submission failed: %d\n", rc);
        return 1;
    }
