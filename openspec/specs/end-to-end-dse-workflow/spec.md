# end-to-end-dse-workflow Specification

## Purpose
Define the complete closed-loop DSE workflow from workload ingestion and architecture catalog instantiation through mapping search, SystemC or gem5+SystemC simulation, evidence export, feedback updates, convergence, and final claim-gated analysis reporting.
## Requirements
...
Every design point or mapping included in the final trusted ranking SHALL have complete SystemC or gem5+SystemC full-flow evidence. L1/L2 analytical/TLM results SHALL be used only for screening, prioritization, or explanation unless explicitly labeled predicted-only. Smoke, bring-up diagnostics, fixed-timing completions, or legacy smoke conversions SHALL NOT satisfy the final trusted evidence gate.
...
#### Scenario: Smoke-only candidate is excluded from trusted ranking
- **WHEN** a candidate has only smoke, bring-up, diagnostic replay, or fixed-timing evidence
- **THEN** report validation prevents it from being selected as best architecture, trusted mapping, or trusted Pareto point

### Requirement: Evidence artifacts are exported for user inspection
Each SystemC or gem5+SystemC run used for decisions SHALL produce a run directory containing manifest, workload package, design point, architecture, mapping, source workload graph, graph lowering report or equivalent executable-view record, simulation request, simulation result, logs, summary traces, and verdict. Debug and forensic artifacts SHALL be generated for finalists, failures, or user-selected runs.

#### Scenario: Summary evidence exists for every trusted run
- **WHEN** a SystemC run contributes to a final conclusion
- **THEN** its run directory contains `manifest.json`, `workload_package.json`, `design_point.json`, `architecture.json`, `mapping.json`, `workload_graph.json`, `graph_lowering_report.json` or an explicit not-needed marker, `simulation_request.json`, `simulation_result.json`, logs, and verdict files
...
- **WHEN** an architecture instance is marked `trusted_final_eligible`
...
#### Scenario: Smoke cannot pass final check
- **WHEN** a candidate cites smoke output as the reason it is complete
- **THEN** report validation fails the trusted claim unless complete full-flow simulation evidence is also present and linked
