#!/usr/bin/env python3
"""Run the DFT nonlocal-projector RTL smoke flow through local/IC-EDA tools."""
...
from dse_v2.reference_workloads.dft_nonlocal_projector_rtl_flow import (  # noqa: E402
...
REMOTE_FILES = ("nonlocal_projector.v", "tb_nonlocal_projector.v", "vivado_synth.tcl", "dc_synth.tcl")
...
def _copy_to_remote(out_dir: Path, *, ssh_target: str, remote_dir: str, timeout_s: int) -> list[dict[str, Any]]:
    attempts = [
        _run(["ssh", ssh_target, f"rm -rf '{remote_dir}' && mkdir -p '{remote_dir}'"], timeout_s=timeout_s)
    ]
    attempts.append(
        _run(
            [
...
def _remote_tool_attempts(out_dir: Path, *, ssh_target: str, remote_dir: str, timeout_s: int) -> list[dict[str, Any]]:
    tool_commands = [
        (
            ...
            (
                "source ~/.bashrc; "
                "vcs -full64 -sverilog nonlocal_projector.v tb_nonlocal_projector.v -o simv > vcs_compile.log 2>&1 "
        ...
        (
            ...
            (
                ...
                "tar czf results_vivado.tgz vivado*.log vivado_*.rpt nonlocal_projector_synth.dcp *_routed.dcp vivado_route_status.rpt vivado_route_timing_summary.rpt .Xil "
        ...
        (
            ...
            (
                ...
                "dc_files=\"$(ls dc_*.rpt dc_stdout.log dc_stderr.log nonlocal_projector_dc_mapped.v dc_synth.ddc command.log 2>/dev/null || true)\"; [ -n \"$dc_files\" ] && tar czf results_dc.tgz $dc_files "
                "2>/dev/null || true"
            ),
            "results_dc.tgz",
        ),
...
def main(argv: list[str] | None = None) -> int:
    ...
    initialize_nonlocal_projector_rtl_flow(out_dir, candidate_id=args.candidate_id)
    remote_dir = args.remote_dir or f"/tmp/dft_accelerate_nonlocal_projector_{out_dir.name}"
    attempts: list[dict[str, Any]] = []
    ...
    evidence = build_nonlocal_projector_evidence_rows(out_dir, environment=f"ssh {args.ssh_target}")
    matrix = write_nonlocal_projector_major_kernel_matrix(
        out_dir,
        candidate_id=args.candidate_id or f"nonlocal_projector_rtl_fpga_smoke::{out_dir.name}",
        evidence_rows=evidence["evidence_rows"],
    )
    status = {
        "schema_version": "dse.dft_scf.nonlocal_projector_rtl_flow_status.v1",
        "status": "passed" if matrix["status"] == "passed" else "blocked",
        "remote_dir": remote_dir,
        "attempts": attempts,
        "evidence_summary": str(out_dir / "nonlocal_projector_rtl_evidence_summary.json"),
        "dft_hardware_evidence_matrix": str(out_dir / "dft_hardware_evidence_matrix.json"),
        "claim_boundary": (
            "This flow can prove only the nonlocal_projector FPGA microkernel smoke gate. "
