#!/usr/bin/env python3
"""Run the DFT kinetic-add RTL smoke flow through local/IC-EDA tools."""
...
from dse_v2.reference_workloads.dft_kinetic_add_rtl_flow import (  # noqa: E402
...
REMOTE_FILES = ("kinetic_add.v", "tb_kinetic_add.v", "vivado_synth.tcl", "dc_synth.tcl")
...
def _remote_tool_attempts(out_dir: Path, *, ssh_target: str, remote_dir: str, timeout_s: int) -> list[dict[str, Any]]:
    tool_commands = [
        (
            ...
            (
                "source ~/.bashrc; "
                "vcs -full64 -sverilog kinetic_add.v tb_kinetic_add.v -o simv > vcs_compile.log 2>&1 "
        ...
        (
            ...
            (
                ...
                "tar czf results_vivado.tgz vivado*.log vivado_*.rpt kinetic_add_synth.dcp *_routed.dcp vivado_route_status.rpt vivado_route_timing_summary.rpt .Xil "
        ...
        (
            ...
            (
                ...
                "dc_files=\"$(ls dc_*.rpt dc_stdout.log dc_stderr.log kinetic_add_dc_mapped.v dc_synth.ddc command.log 2>/dev/null || true)\"; [ -n \"$dc_files\" ] && tar czf results_dc.tgz $dc_files "
                "2>/dev/null || true"
            ),
            "results_dc.tgz",
        ),
...
def main(argv: list[str] | None = None) -> int:
    ...
    evidence = build_kinetic_add_evidence_rows(out_dir, environment=f"ssh {args.ssh_target}")
    matrix = write_kinetic_add_major_kernel_matrix(
        out_dir,
        candidate_id=args.candidate_id or f"kinetic_add_rtl_fpga_smoke::{out_dir.name}",
        evidence_rows=evidence["evidence_rows"],
    )
    status = {
        "schema_version": "dse.dft_scf.kinetic_add_rtl_flow_status.v1",
        "status": "passed" if matrix["status"] == "passed" else "blocked",
        "remote_dir": remote_dir,
        "attempts": attempts,
        "evidence_summary": str(out_dir / "kinetic_add_rtl_evidence_summary.json"),
        "dft_hardware_evidence_matrix": str(out_dir / "dft_hardware_evidence_matrix.json"),
        "claim_boundary": (
            "This flow can prove only the kinetic_add FPGA microkernel smoke gate. "
