#!/usr/bin/env python3
"""Run the DFT DMA/HBM movement RTL smoke flow through local/IC-EDA tools."""
...
from dse_v2.reference_workloads.dft_dma_hbm_rtl_flow import (  # noqa: E402
    ...
    tool_commands = [
        ...
        (
            ...
            (
                ...
                "tar czf results_vivado.tgz vivado*.log vivado_*.rpt dma_hbm_movement_engine_synth.dcp *_routed.dcp vivado_route_status.rpt vivado_route_timing_summary.rpt .Xil "
        ...
        (
            ...
            (
                ...
                "dc_files=\"$(ls dc_*.rpt dc_stdout.log dc_stderr.log dma_hbm_movement_engine_dc_mapped.v dc_synth.ddc command.log 2>/dev/null || true)\"; [ -n \"$dc_files\" ] && tar czf results_dc.tgz $dc_files "
                "2>/dev/null || true"
            ),
            "results_dc.tgz",
        ),
...
def main(argv: list[str] | None = None) -> int:
    ...
    initialize_dma_hbm_rtl_flow(out_dir, candidate_id=args.candidate_id)
    remote_dir = args.remote_dir or f"/tmp/dft_accelerate_dma_hbm_movement_engine_{out_dir.name}"
    attempts: list[dict[str, Any]] = []
    ...
    evidence = build_dma_hbm_evidence_rows(out_dir, environment=f"ssh {args.ssh_target}")
    matrix = write_dma_hbm_major_kernel_matrix(
        out_dir,
        candidate_id=args.candidate_id or f"dma_hbm_movement_engine_rtl_fpga_smoke::{out_dir.name}",
        evidence_rows=evidence["evidence_rows"],
    )
    status = {
        "schema_version": "dse.dft_scf.dma_hbm_movement_engine_rtl_flow_status.v1",
        "status": "passed" if matrix["status"] == "passed" else "blocked",
        "remote_dir": remote_dir,
        "attempts": attempts,
        "evidence_summary": str(out_dir / "dma_hbm_movement_engine_rtl_evidence_summary.json"),
        "dft_hardware_evidence_matrix": str(out_dir / "dft_hardware_evidence_matrix.json"),
        "claim_boundary": (
            "This flow can prove only the dma_hbm_movement_engine FPGA microkernel smoke gate. "
