#!/usr/bin/env python3
"""Run the DFT reduction-dot RTL smoke flow through local/IC-EDA tools."""
...
from dse_v2.reference_workloads.dft_reduction_dot_rtl_flow import (  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    ...
    initialize_reduction_dot_tree_rtl_flow(out_dir, candidate_id=args.candidate_id)
    remote_dir = args.remote_dir or f"/tmp/dft_accelerate_reduction_dot_tree_{out_dir.name}"
    attempts: list[dict[str, Any]] = []
    ...
    evidence = build_reduction_dot_tree_evidence_rows(out_dir, environment=f"ssh {args.ssh_target}")
    matrix = write_reduction_dot_tree_major_kernel_matrix(
        out_dir,
        candidate_id=args.candidate_id or f"reduction_dot_tree_rtl_fpga_smoke::{out_dir.name}",
        evidence_rows=evidence["evidence_rows"],
    )
    status = {
        "schema_version": "dse.dft_scf.reduction_dot_tree_rtl_flow_status.v1",
        "status": "passed" if matrix["status"] == "passed" else "blocked",
        "remote_dir": remote_dir,
        "attempts": attempts,
        "evidence_summary": str(out_dir / "reduction_dot_tree_rtl_evidence_summary.json"),
        "dft_hardware_evidence_matrix": str(out_dir / "dft_hardware_evidence_matrix.json"),
        "claim_boundary": (
            "This flow can prove only the reduction_dot_tree FPGA microkernel smoke gate. "
