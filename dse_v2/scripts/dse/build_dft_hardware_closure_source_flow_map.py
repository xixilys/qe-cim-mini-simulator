#!/usr/bin/env python3
"""Build a fail-closed DFT hardware-closure source_flow_map.json from source roots."""
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="Generated source-flow root to scan; may be passed multiple times.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    status = write_dft_hardware_closure_source_flow_map(
        ...
        source_flow_roots=args.source_flow_root,
    )
    if not args.quiet:
