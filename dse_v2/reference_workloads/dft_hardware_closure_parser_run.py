...
def _parse_stage(stage_id: str, paths: list[Path]) -> tuple[str, Dict[str, Any], str, list[str]]:
    ...
    if stage_id == "hls_or_rtl_synth":
        for payload in payloads:
            for key in ("lut", "ff", "bram", "dsp", "latency_cycles"):
                if key in payload:
                    metrics[key] = payload[key]
        # Prefer the structured synthesis payload generated by raw-stage
        # materialization.  A successful Vivado implementation transcript can
        # contain intermediate router diagnostics such as "Failed Nets" before
        # route_design completes; those strings must not override the
        # synth-stage JSON verdict.
        json_vote = _json_vote(payloads)
        if json_vote:
            return json_vote, metrics, parser_id, []
        text = "\n".join(_text(path).upper() for path in paths)
        if "SYNTH_DESIGN FAILED" in text:
            return "failed", metrics, parser_id, []
        if "SYNTH_DESIGN COMPLETED SUCCESSFULLY" in text:
            return "passed", metrics, parser_id, []
        return vote or "inconclusive", metrics, parser_id, []
    if stage_id == "vivado_fpga_synth_or_impl":
        ...
        if any(
            ...
            or str(payload.get("verdict", payload.get("status", ""))).lower()
            in {"fail", "failed", "error", "violated"}
            for payload in route_payloads
        ):
            vote = "failed"
        else:
            # Route completion is a stronger Vivado-stage signal than generic
            # text markers.  Do not let benign route-progress phrases such as
            # "Number of Failed Nets" turn a completed route into a failed gate.
            vote = "passed" if route_completed or timing_met_by_log else vote
