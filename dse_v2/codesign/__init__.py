"""HW/SW co-design candidate and evidence artifact helpers."""

from .artifacts import (
    CODESIGN_L4_EVIDENCE_ARTIFACTS,
    CODESIGN_STEP2_ARTIFACTS,
    build_codesign_l4_evidence,
    build_default_codesign_artifacts,
    split_codesign_artifacts,
    validate_codesign_artifacts,
)
from .l4_closure import (
    L4_INTERFACE_METRICS_SCHEMA,
    canonicalize_l4_interface_metrics,
)
from .hardware_claim_gates import (
    HARDWARE_CLAIM_GATE_SCHEMA,
    HARDWARE_CLAIM_REQUIREMENTS,
    validate_hardware_claim_evidence,
)
from .dft_hardware_evidence import (
    DFT_HARDWARE_EVIDENCE_SCHEMA,
    FIRST_PROTOTYPE_HOST_BOUND_STAGES,
    IC_EDA_TOOL_AVAILABILITY_SCHEMA,
    MAJOR_SCF_KERNEL_IDS,
    MAJOR_SCF_KERNELS,
    REQUIRED_IC_EDA_TOOLS,
    build_ic_eda_tool_availability_report,
    build_major_kernel_evidence_matrix,
    run_tool_probe_commands,
)
from .dft_l4_goal_binding import (
    DFT_L4_GOAL_BINDING_SCHEMA,
    build_dft_l4_goal_binding,
    validate_dft_l4_goal_binding,
    write_dft_l4_goal_binding,
)
from .evidence_ledger import (
    LEDGER_SCHEMA,
    LEDGER_VALIDATION_SCHEMA,
    REQUIRED_EVIDENCE_REFS,
    REQUIRED_ROW_FIELDS,
    artifact_ref,
    sha256_file,
    validate_candidate_evidence_ledger,
)
from .dft_scf_workstreams import (
    STRICT_BUNDLE_REQUIRED_ASSETS,
    STRICT_DFT_QE_WORKLOAD_CLASSES,
    adjudicate_hardware_claim_evidence,
    build_full_scf_hybrid_step5_report,
    build_wave15_trace,
    candidate_release_lane,
    candidate_release_policy,
    formal_pareto_candidates,
    validate_artifact_scope_ids,
    validate_step5_full_scf_cost_report,
    validate_strict_dft_qe_bundle,
)
from .complete_dse_search_space import (
    IDENTITY_LAYER_KEYS,
    NON_IDENTITY_FIELDS,
    build_architecture_search_space,
    build_candidate_record,
    build_freeze_gate_verdict,
    build_release_pruning_rationale_report,
    build_release_subset_manifest,
    build_schedule_legality_report,
    build_workload_architecture_prior_report,
    classify_candidate_legality,
    complete_dse_candidate_id,
    validate_architecture_search_space,
    write_complete_dse_search_space_artifacts,
)
from .qe_callgraph_offload_search import (
    OFFLOAD_IDENTITY_LAYER_KEYS,
    OFFLOAD_TARGET_LAYER,
    build_l4_offload_attempt_queue,
    build_offload_candidate_record,
    build_offload_bundle_search_space,
    build_offload_opportunity_manifest,
    build_offload_target_identity_schema,
    build_offload_value_l4_evidence_matrix,
    build_offload_value_report,
    build_bundle_runtime_contract_report,
    build_bundle_harness_readiness_report,
    build_qe_callgraph_inventory,
    build_qe_callsite_patch_manifest,
    classify_l4_offload_value,
    complete_dse_offload_candidate_id,
    validate_qe_callsite_patch_manifest,
    write_qe_callgraph_offload_search_artifacts,
)

__all__ = [
    "CODESIGN_L4_EVIDENCE_ARTIFACTS",
    "CODESIGN_STEP2_ARTIFACTS",
    "HARDWARE_CLAIM_GATE_SCHEMA",
    "HARDWARE_CLAIM_REQUIREMENTS",
    "DFT_HARDWARE_EVIDENCE_SCHEMA",
    "DFT_L4_GOAL_BINDING_SCHEMA",
    "FIRST_PROTOTYPE_HOST_BOUND_STAGES",
    "IC_EDA_TOOL_AVAILABILITY_SCHEMA",
    "LEDGER_SCHEMA",
    "LEDGER_VALIDATION_SCHEMA",
    "L4_INTERFACE_METRICS_SCHEMA",
    "REQUIRED_EVIDENCE_REFS",
    "MAJOR_SCF_KERNEL_IDS",
    "MAJOR_SCF_KERNELS",
    "REQUIRED_IC_EDA_TOOLS",
    "REQUIRED_ROW_FIELDS",
    "STRICT_BUNDLE_REQUIRED_ASSETS",
    "STRICT_DFT_QE_WORKLOAD_CLASSES",
    "IDENTITY_LAYER_KEYS",
    "NON_IDENTITY_FIELDS",
    "OFFLOAD_IDENTITY_LAYER_KEYS",
    "OFFLOAD_TARGET_LAYER",
    "adjudicate_hardware_claim_evidence",
    "artifact_ref",
    "build_architecture_search_space",
    "build_candidate_record",
    "build_codesign_l4_evidence",
    "build_default_codesign_artifacts",
    "build_dft_l4_goal_binding",
    "build_full_scf_hybrid_step5_report",
    "build_freeze_gate_verdict",
    "build_ic_eda_tool_availability_report",
    "build_l4_offload_attempt_queue",
    "build_major_kernel_evidence_matrix",
    "build_offload_candidate_record",
    "build_offload_bundle_search_space",
    "build_offload_opportunity_manifest",
    "build_offload_target_identity_schema",
    "build_offload_value_l4_evidence_matrix",
    "build_offload_value_report",
    "build_bundle_runtime_contract_report",
    "build_bundle_harness_readiness_report",
    "build_qe_callgraph_inventory",
    "build_qe_callsite_patch_manifest",
    "canonicalize_l4_interface_metrics",
    "build_release_pruning_rationale_report",
    "build_release_subset_manifest",
    "build_schedule_legality_report",
    "build_wave15_trace",
    "build_workload_architecture_prior_report",
    "candidate_release_lane",
    "candidate_release_policy",
    "classify_candidate_legality",
    "classify_l4_offload_value",
    "complete_dse_candidate_id",
    "complete_dse_offload_candidate_id",
    "formal_pareto_candidates",
    "run_tool_probe_commands",
    "sha256_file",
    "split_codesign_artifacts",
    "validate_architecture_search_space",
    "validate_artifact_scope_ids",
    "validate_candidate_evidence_ledger",
    "validate_codesign_artifacts",
    "validate_dft_l4_goal_binding",
    "validate_hardware_claim_evidence",
    "validate_qe_callsite_patch_manifest",
    "write_complete_dse_search_space_artifacts",
    "write_dft_l4_goal_binding",
    "write_qe_callgraph_offload_search_artifacts",
]
