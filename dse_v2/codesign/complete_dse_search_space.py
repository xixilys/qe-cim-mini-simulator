...
RESEARCH_ONLY_CLAIMS = ("research_projection", "screening_projection", "blocked")
...
BANNED_COMPLETION_SUBSETS = ("top_k", "representative", "pareto", "promoted_only")
...
def _indexed_by_id(rows: Iterable[Mapping[str, Any]], key: str = "id") -> Dict[str, Mapping[str, Any]]:
...
def canonical_candidate_identity(identity_layers: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    """
    missing = [layer for layer in IDENTITY_LAYER_KEYS if layer not in identity_layers]
    if missing:
        raise ValueError(f"missing candidate identity layers: {', '.join(missing)}")

    forbidden = sorted(set(identity_layers).intersection(NON_IDENTITY_FIELDS))
    if forbidden:
        raise ValueError(f"non-identity fields supplied as identity layers: {', '.join(forbidden)}")
    ...
    base_rows = [
        {
            ...
            "preferred_workload_features": ["fft", "rho", "potential_update", "producer_consumer"],
        ...
        },
        {
            ...
            "release_v1_status": "required",
            "preferred_workload_features": ["residual", "mix_rho", "vector_update", "reduction"],
            "claim_boundary": "architecture identity only; evidence rows close separately",
        },
        {
            "id": "spatial_pe_array",
            ...
            "release_v1_status": "required",
            "preferred_workload_features": ["h_psi", "s_psi", "build_H_sub", "dense_block"],
            "claim_boundary": "architecture identity only; evidence rows close separately",
        },
        {
            ...
            "release_v1_status": "required",
            "preferred_workload_features": ["multi_kernel_overlap", "heterogeneous_qe_iteration"],
            "claim_boundary": "architecture identity only; evidence rows close separately",
        },
    ]
    hybrid_rows = [
...
def build_hybrid_template_manifest() -> Dict[str, Any]:
    ...
    hybrids = [dict(row) for row in taxonomy["entries"] if row["kind"] == "hybrid_template"]
    for row in hybrids:
    ...
    families = [
        {
            ...
            "bounded_parameters": {"fft_batching": ["per_band"], "rho_accumulation": ["streamed"]},
            "compatible_taxonomy_ids": ["streaming_pipeline", "pipeline_simd_fused", "pipeline_task_overlap"],
        },
        {
            "id": "blocked_dense_subspace",
            "family": "dense_subspace",
            "variant": "blocked_hpsi_spsi_subspace",
            "bounded_parameters": {"block_shape": ["band_block_16"], "solver_policy": ["direct_blocked"]},
            "compatible_taxonomy_ids": ["spatial_pe_array", "pipeline_spatial_array", "task_parallel_spatial_array"],
        },
        {
            "id": "vector_residual_mixing",
            "family": "vector_update",
            "variant": "residual_mix_reduce",
            "bounded_parameters": {"lane_group": ["simd_8"], "reduction_tree": ["pairwise"]},
            "compatible_taxonomy_ids": ["simd_vector", "pipeline_simd_fused", "task_parallel_simd"],
        },
        {
            "id": "qe_task_graph_overlap",
            "family": "task_graph",
            "variant": "heterogeneous_kernel_overlap",
            "bounded_parameters": {"graph_window": ["one_qe_iteration"], "fallback": ["host_per_kernel"]},
            "compatible_taxonomy_ids": ["task_parallel_engines", "task_parallel_simd", "task_parallel_spatial_array", "pipeline_task_overlap"],
        },
    ]
    payload = {
    ...
    schedules = [
        {
            ...
            "compatible_taxonomy_ids": ["streaming_pipeline", "simd_vector", "spatial_pe_array"],
        },
        {
            ...
            "compatible_taxonomy_ids": ["streaming_pipeline", "pipeline_simd_fused", "pipeline_spatial_array"],
        },
        {
            ...
            "compatible_taxonomy_ids": ["task_parallel_engines", "task_parallel_simd", "task_parallel_spatial_array"],
        },
        {
            ...
            "compatible_taxonomy_ids": ["pipeline_task_overlap", "pipeline_simd_fused", "pipeline_spatial_array"],
        },
    ]
    payload = {
...
def build_legality_constraints_manifest() -> Dict[str, Any]:
    ...
    payload["constraints_hash"] = _stable_hash_without(payload, "constraints_hash")
    return payload
...
def _identity_from_seed(seed: Mapping[str, str]) -> Dict[str, Dict[str, Any]]:
    ...
    return {
        ...
        "mapping_layout_parameters": {
            ...
            "memory_hierarchy_target": indices["mapping"][mapping_id]["memory_hierarchy_target"],
        },
        "compile_time_schedule_parameters": {
        ...
        "runtime_scheduling_parameters": {
            ...
            "engine_assignment": indices["runtime"][runtime_id]["engine_assignment"],
        },
    }
...
def classify_candidate_legality(identity_layers: Mapping[str, Any]) -> tuple[bool, list[str]]:
    reasons: list[str] = []
    try:
    ...
    compile_id = str(layers["compile_time_schedule_parameters"].get("compile_schedule_id", ""))
    runtime_id = str(layers["runtime_scheduling_parameters"].get("runtime_schedule_id", ""))

    if taxonomy_id not in indices["taxonomy"]:
        reasons.append("taxonomy_id is not a predeclared release-v1 base family or hybrid template")
    if "+" in taxonomy_id or "x" in taxonomy_id:
        reasons.append("arbitrary base-family Cartesian composites are illegal for release v1")
    ...
    compatibility_checks = [
        ("algorithm", algorithm_id, "algorithm is not compatible with architecture taxonomy"),
        ("mapping", mapping_id, "mapping/layout is not compatible with architecture taxonomy"),
        ("compile", compile_id, "compile-time schedule is not compatible with architecture taxonomy"),
        ("runtime", runtime_id, "runtime schedule is not compatible with architecture taxonomy"),
    ]
    for index_name, item_id, reason in compatibility_checks:
        compatible = indices[index_name][item_id].get("compatible_taxonomy_ids", [])
...
def build_research_space_manifest() -> Dict[str, Any]:
    payload = {
        ...
        "search_methods": ["hierarchical_generators", "legality_pruning", "surrogate_screening", "multi_fidelity_promotion"],
...
def build_release_subset_manifest(seed_rows: Sequence[Mapping[str, str]] | None = None) -> Dict[str, Any]:
    ...
    payload = {
        ...
        "included_taxonomy_ids": [
            candidate["identity"]["identity_layers"]["architecture_parameters"]["taxonomy_id"]
...
def build_candidate_generation_report(release_subset: Mapping[str, Any] | None = None) -> Dict[str, Any]:
    subset = dict(release_subset or build_release_subset_manifest())
    candidates = [row for row in subset.get("candidates", []) if isinstance(row, Mapping)]
    all_have_layers = all(
        tuple(candidate.get("identity", {}).get("identity_layers", {}).keys()) == IDENTITY_LAYER_KEYS
        for candidate in candidates
    )
    payload = {
...
def build_legality_pruning_report(release_subset: Mapping[str, Any] | None = None) -> Dict[str, Any]:
    subset = dict(release_subset or build_release_subset_manifest())
    ...
    _, illegal_probe_reasons = classify_candidate_legality(illegal_probe_identity)
    arbitrary_identity = {
    ...
    payload = {
        ...
        "all_pruned_rows_have_stable_reason": all(row.get("reasons") or row.get("reason") for row in pruned_rows),
        "allowed_pruning_classifications": ["illegal", "research_only", "over_budget", "blocked"],
...
def build_release_l4_runtime_cost_report(
    ...
    *,
    ...
    status = "passed" if rows <= int(budget["l4_evidence_rows_hard_cap"]) else "blocked"
    payload = {
...
def _missing_required_taxonomy_ids(release_subset: Mapping[str, Any]) -> list[str]:
    included = set(str(item) for item in release_subset.get("included_taxonomy_ids", []) or [])
    return [taxonomy_id for taxonomy_id in REQUIRED_TAXONOMY_IDS if taxonomy_id not in included]
...
def build_freeze_gate_verdict(
    ...
    *,
    ...
    subset = dict(release_subset or build_release_subset_manifest())
    policy = dict(selection_policy or {"selection_kind": "predeclared_finite_release_subset"})
    ...
    legal_count = int(subset.get("legal_candidate_count") or 0)
    hard_cap = int(budget_payload.get("legal_release_candidates_hard_cap") or 0)
    if legal_count <= 0:
        blockers.append({"id": "no_legal_candidates", "reason": "release subset has no legal candidates"})
    if hard_cap and legal_count > hard_cap:
        blockers.append({"id": "candidate_count_over_hard_cap", "count": legal_count, "hard_cap": hard_cap})

    missing = _missing_required_taxonomy_ids(subset)
    if missing:
        blockers.append({"id": "missing_required_taxonomy_entries", "missing": missing})

    selection_kind = str(policy.get("selection_kind", ""))
    if selection_kind in BANNED_COMPLETION_SUBSETS:
        blockers.append({"id": "downgraded_subset_selection", "selection_kind": selection_kind})
    if policy.get("fixed_candidate_only") is True:
        blockers.append({"id": "fixed_candidate_only_downgrade", "reason": "fixed candidate lists cannot replace a generated release universe"})

    research_count = int(research_payload.get("estimated_broad_candidate_count") or 0)
    ratio = (legal_count / research_count) if research_count else 1.0
    ratio_cap = float(budget_payload.get("research_to_release_ratio_cap") or 1.0)
    if ratio > ratio_cap:
        blockers.append({"id": "release_subset_not_ambitiously_pruned", "ratio": ratio, "cap": ratio_cap})
    ...
    if not all_have_layers:
        blockers.append({"id": "missing_identity_layer", "reason": "all five identity layers are required before freeze"})
...
def build_architecture_search_space() -> Dict[str, Any]:
    ...
    payload = {
        ...
        "candidate_generation_report": build_candidate_generation_report(release_subset),
        "legality_pruning_report": build_legality_pruning_report(release_subset),
        "release_cardinality_budget": build_release_cardinality_budget(),
        "release_l4_runtime_cost_report": build_release_l4_runtime_cost_report(release_subset),
...
def validate_architecture_search_space(search_space: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    checks = {
        "has_two_tiers": set(search_space.get("search_space_schema", {}).get("tiers", {})) == {"research_space", "release_subset"},
        "candidate_identity_complete": candidate_report.get("all_candidates_have_all_identity_layers") is True,
        "required_taxonomy_present": not _missing_required_taxonomy_ids(subset if isinstance(subset, Mapping) else {}),
        "freeze_gate_passed": freeze.get("status") == "passed",
        "no_completion_claim": search_space.get("claim_boundary", "").endswith("no trusted speedup or completion claim"),
    }
    payload = {
...
def artifact_bundle() -> Dict[str, Dict[str, Any]]:
    ...
    return {
        ...
        "hybrid_template_manifest.json": search_space["hybrid_template_manifest"],
        "algorithm_family_manifest.json": search_space["algorithm_family_manifest"],
        ...
        "research_space_manifest.json": search_space["research_space_manifest"],
        "architecture_prior_seed_manifest.json": search_space["architecture_prior_seed_manifest"],
        "release_subset_manifest.json": search_space["release_subset_manifest"],
        "candidate_generation_report.json": search_space["candidate_generation_report"],
        "legality_pruning_report.json": search_space["legality_pruning_report"],
        "release_cardinality_budget.json": search_space["release_cardinality_budget"],
        "release_l4_runtime_cost_report.json": search_space["release_l4_runtime_cost_report"],
        "freeze_gate_verdict.json": search_space["freeze_gate_verdict"],
        "architecture_search_space.json": search_space,
        "validation_report.json": validate_architecture_search_space(search_space),
    }
