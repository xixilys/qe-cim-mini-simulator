#!/usr/bin/env python3
"""Targeted checks for the DFT reduction/dot RTL evidence lane."""
...
from dse_v2.reference_workloads.dft_reduction_dot_rtl_flow import (
    REDUCTION_DOT_KERNEL_ID,
    build_reduction_dot_tree_evidence_rows,
    default_kernel_dispositions,
    initialize_reduction_dot_tree_rtl_flow,
    write_reduction_dot_tree_major_kernel_matrix,
)
from dse_v2.scripts.dse.run_dft_reduction_dot_rtl_flow import main as run_reduction_dot_flow
...
def test_reduction_dot_rtl_flow_generates_deterministic_sources_and_golden(tmp_path):
    initialized = initialize_reduction_dot_tree_rtl_flow(tmp_path)

    assert initialized["manifest"]["kernel_id"] == REDUCTION_DOT_KERNEL_ID
    assert (tmp_path / "reduction_dot_tree.v").exists()
    assert (tmp_path / "tb_reduction_dot_tree.v").exists()
    assert "module reduction_dot_tree" in (tmp_path / "reduction_dot_tree.v").read_text(encoding="utf-8")
    assert "REDUCTION_DOT_RTL_PASS" in (tmp_path / "tb_reduction_dot_tree.v").read_text(encoding="utf-8")
    golden = json.loads((tmp_path / "golden_correctness.json").read_text(encoding="utf-8"))
    assert golden["kernel_id"] == REDUCTION_DOT_KERNEL_ID
    assert golden["status"] == "passed"
    assert golden["expected"] == {"dot": -42}


def test_reduction_dot_rtl_flow_is_fail_closed_before_remote_tool_outputs(tmp_path):
    initialize_reduction_dot_tree_rtl_flow(tmp_path)

    evidence = build_reduction_dot_tree_evidence_rows(tmp_path)
    matrix = write_reduction_dot_tree_major_kernel_matrix(
        tmp_path,
        candidate_id="unit-reduction-dot-before-tools",
        evidence_rows=evidence["evidence_rows"],
    )

    assert {row["kernel_id"] for row in evidence["evidence_rows"]} == {REDUCTION_DOT_KERNEL_ID}
    ...
    reduction_row = next(row for row in matrix["kernel_rows"] if row["kernel_id"] == REDUCTION_DOT_KERNEL_ID)
...
def test_reduction_dot_rtl_flow_builds_fpga_matrix_and_cannot_satisfy_other_kernel_ids(tmp_path):
    ...
    matrix = write_reduction_dot_tree_major_kernel_matrix(
        tmp_path,
        candidate_id="unit-reduction-dot-fpga-smoke",
        evidence_rows=evidence["evidence_rows"],
    )
    ...
    reduction_row = next(row for row in matrix["kernel_rows"] if row["kernel_id"] == REDUCTION_DOT_KERNEL_ID)
    assert reduction_row["claim_allowed"] is True
    ...
    kinetic_matrix = build_major_kernel_evidence_matrix(
        ...
        evidence_rows=evidence["evidence_rows"],
        candidate_id="unit-reduction-dot-not-kinetic",
    )
...
def test_reduction_dot_rtl_cli_writes_replayable_blocked_status(tmp_path):
    out_dir = tmp_path / "run"

    candidate_id = "release-cand-reduction-dot"
    result = run_reduction_dot_flow(["--out", str(out_dir), "--candidate-id", candidate_id, "--skip-remote", "--allow-blocked"])

    assert result == 0
    status = json.loads((out_dir / "status.json").read_text(encoding="utf-8"))
    assert status["status"] == "blocked"
    assert status["attempts"] == []
    _assert_candidate_source_flow(out_dir, candidate_id=candidate_id, kernel_id=REDUCTION_DOT_KERNEL_ID)
...
def test_reduction_dot_source_flow_plan_blocks_missing_or_wrong_candidate_id(tmp_path):
    _assert_source_flow_candidate_blockers(
        tmp_path,
        initialize_flow=initialize_reduction_dot_tree_rtl_flow,
        candidate_id="release-cand-reduction-dot",
