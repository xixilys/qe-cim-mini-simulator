#!/usr/bin/env python3
"""Targeted checks for the DFT DMA/HBM movement RTL evidence lane."""
...
from dse_v2.reference_workloads.dft_dma_hbm_rtl_flow import (
    DMA_HBM_KERNEL_ID,
    build_dma_hbm_evidence_rows,
    default_kernel_dispositions,
    initialize_dma_hbm_rtl_flow,
    write_dma_hbm_major_kernel_matrix,
)
from dse_v2.scripts.dse.run_dft_dma_hbm_rtl_flow import main as run_dma_hbm_flow
...
def test_dma_hbm_rtl_flow_generates_deterministic_sources_and_golden(tmp_path):
    initialized = initialize_dma_hbm_rtl_flow(tmp_path)

    assert initialized["manifest"]["kernel_id"] == DMA_HBM_KERNEL_ID
    assert (tmp_path / "dma_hbm_movement_engine.v").exists()
    assert (tmp_path / "tb_dma_hbm_movement_engine.v").exists()
    rtl = (tmp_path / "dma_hbm_movement_engine.v").read_text(encoding="utf-8")
    tb = (tmp_path / "tb_dma_hbm_movement_engine.v").read_text(encoding="utf-8")
    assert "module dma_hbm_movement_engine" in rtl
    assert "DMA_HBM_RTL_PASS" in tb
    assert "copy_word(2'b10" in tb
    assert "copy_word(2'b11" in tb
    golden = json.loads((tmp_path / "golden_correctness.json").read_text(encoding="utf-8"))
    assert golden["kernel_id"] == DMA_HBM_KERNEL_ID
    assert golden["status"] == "passed"
    assert golden["expected"]["burst_copy"] == {"20": 4098, "21": 4099, "22": 4100, "23": 4101}
    assert golden["expected"]["gather_copy"] == {"20": 4101, "21": 4104, "22": 4107}
    assert golden["expected"]["scatter_copy"] == {"40": 4098, "42": 4099, "44": 4100}


def test_dma_hbm_rtl_flow_is_fail_closed_before_remote_tool_outputs(tmp_path):
    initialize_dma_hbm_rtl_flow(tmp_path)

    evidence = build_dma_hbm_evidence_rows(tmp_path)
    matrix = write_dma_hbm_major_kernel_matrix(
        tmp_path,
        candidate_id="unit-dma-hbm-before-tools",
        evidence_rows=evidence["evidence_rows"],
    )

    assert {row["kernel_id"] for row in evidence["evidence_rows"]} == {DMA_HBM_KERNEL_ID}
    ...
    dma_row = next(row for row in matrix["kernel_rows"] if row["kernel_id"] == DMA_HBM_KERNEL_ID)
    assert dma_row["hardware_claim_gate"]["missing_or_blocked_stages"] == [
        "hls_or_rtl_sim",
        "hls_or_rtl_synth",
...
def test_dma_hbm_rtl_flow_builds_fpga_matrix_and_cannot_satisfy_other_kernel_ids(tmp_path):
    initialize_dma_hbm_rtl_flow(tmp_path)
    (tmp_path / "vcs_run.log").write_text("DMA_HBM_RTL_PASS burst/stride/gather/scatter ok\n", encoding="utf-8")
    (tmp_path / "vivado_stdout.log").write_text("synth_design completed successfully\n", encoding="utf-8")
    (tmp_path / "vivado_utilization.rpt").write_text("LUT as Logic | 24\n", encoding="utf-8")
    ...
    matrix = write_dma_hbm_major_kernel_matrix(
        tmp_path,
        candidate_id="unit-dma-hbm-fpga-smoke",
        evidence_rows=evidence["evidence_rows"],
    )
    ...
    dma_row = next(row for row in matrix["kernel_rows"] if row["kernel_id"] == DMA_HBM_KERNEL_ID)
    assert dma_row["claim_allowed"] is True
    reduction_dispositions = default_kernel_dispositions(accelerated_kernel_id="reduction_dot_tree")
    reduction_matrix = build_major_kernel_evidence_matrix(
        reduction_dispositions,
        evidence_rows=evidence["evidence_rows"],
        candidate_id="unit-dma-hbm-not-reduction",
    )
    ...
    assert reduction_row["hardware_claim_gate"]["missing_or_blocked_stages"] == [
        "golden_correctness",
        "hls_or_rtl_sim",
        "hls_or_rtl_synth",
        "vivado_fpga_synth_or_impl",
...
def test_dma_hbm_rtl_cli_writes_replayable_blocked_status(tmp_path):
    out_dir = tmp_path / "run"

    candidate_id = "release-cand-dma-hbm"
    result = run_dma_hbm_flow(["--out", str(out_dir), "--candidate-id", candidate_id, "--skip-remote", "--allow-blocked"])

    assert result == 0
    status = json.loads((out_dir / "status.json").read_text(encoding="utf-8"))
    assert status["status"] == "blocked"
    assert status["attempts"] == []
    _assert_candidate_source_flow(out_dir, candidate_id=candidate_id, kernel_id=DMA_HBM_KERNEL_ID)
    assert (out_dir / "manifest.json").exists()
    assert (out_dir / "evidence_rows.json").exists()
    assert (out_dir / "dft_hardware_evidence_matrix.json").exists()

def test_dma_hbm_asic_dc_blocks_when_valid_reports_target_library_but_ddc_missing(tmp_path):
    initialize_dma_hbm_rtl_flow(tmp_path)
    (tmp_path / "vcs_run.log").write_text("DMA_HBM_RTL_PASS burst/stride/gather/scatter ok\n", encoding="utf-8")
    (tmp_path / "vivado_stdout.log").write_text("synth_design completed successfully\n", encoding="utf-8")
...
def test_dma_hbm_source_flow_plan_blocks_missing_or_wrong_candidate_id(tmp_path):
    _assert_source_flow_candidate_blockers(
        tmp_path,
        initialize_flow=initialize_dma_hbm_rtl_flow,
        candidate_id="release-cand-dma-hbm",
