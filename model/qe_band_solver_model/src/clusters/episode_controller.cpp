...
EpisodeControllerState EpisodeController::begin_episode(
  ...
  if (!descriptor.graph_frontdoor_mode.empty()) {
    state.controller_mode = "GRAPH_FRONTDOOR_PROJECTED_EPISODE";
    state.fifo_ab_credit_limit += std::min(2, descriptor.graph_flow_count / 2);
    state.fifo_bc_credit_limit += std::min(1, descriptor.graph_leaf_component_count / 8);
    if (!descriptor.graph_has_refresh_unit) {
      state.fifo_cd_credit_limit = 0;
    } else {
      state.fifo_cd_credit_limit += 1;
    }
    if (!descriptor.graph_has_reduction_unit) {
      state.fifo_bc_credit_limit = 0;
    }
    state.controller_sync_ref_cycles += std::min(4, descriptor.graph_flow_count);
  }
  ...
  if (is_experimental_signature_case(descriptor)) {
    if (descriptor.support_grid_mode != "BYPASS") {
      state.estimated_resident_footprint_kib += 12.0;
    }
    if (descriptor.projector_pressure == "high") {
      state.estimated_resident_footprint_kib += 16.0;
    }
    if (descriptor.generalized_ratio_bucket == "high") {
      state.estimated_resident_footprint_kib += 12.0;
    }
    if (descriptor.post_scf_extension_level == "mobility_extension_expected" ||
        descriptor.post_scf_extension_level == "epw_expected") {
      state.estimated_resident_footprint_kib += 24.0;
      state.controller_sync_ref_cycles += 1;
    }
    if (descriptor.workload_topology == "slab_interface") {
      state.fifo_ab_credit_limit = std::max(1, state.fifo_ab_credit_limit - 1);
    }
  }
  if (!descriptor.graph_frontdoor_mode.empty()) {
    state.estimated_resident_footprint_kib +=
        2.0 * static_cast<double>(descriptor.graph_leaf_component_count) +
        0.5 * static_cast<double>(descriptor.graph_module_count);
    if (!descriptor.graph_has_leaf_hotpath_flow) {
      state.estimated_resident_footprint_kib =
          std::max(48.0, state.estimated_resident_footprint_kib - 18.0);
    }
    if (descriptor.graph_has_vector_diag_companion) {
      state.controller_sync_ref_cycles += 1;
    }
    if (!descriptor.graph_has_fft_unit) {
      state.estimated_resident_footprint_kib =
          std::max(64.0, state.estimated_resident_footprint_kib - 12.0);
    }
  }
  state.resident_fit =
