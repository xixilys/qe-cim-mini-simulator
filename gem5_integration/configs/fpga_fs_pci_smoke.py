...
if cpu_type == 'atomic':
    default_mem_mode = 'atomic'
elif cpu_type == 'kvm':
    default_mem_mode = 'atomic_noncaching'
else:
    default_mem_mode = 'timing'
...
kernel_path = os.environ.get(
    'GEM5_KERNEL_PATH',
    f'{home}/.cache/gem5/x86-linux-kernel-6.8.0-52-generic-1.0.0',
)
initramfs_path = os.environ.get(
    'GEM5_INITRAMFS_PATH',
    os.path.join(script_dir, '..', 'minimal_rootfs', 'initramfs.cpio.gz'),
)
...
if cpu_type == 'atomic':
    ...
    system.kvm_vm = KvmVM()
    system.cpu = X86KvmCPU(cpu_id=0, usePerf=False)
...
if cpu_type == 'kvm':
    # Keep the smoke in single-eventq mode.  The usual multi-eventq KVM setup
    # is faster, but this minimal x86/PCI topology can otherwise trip cross
    # event-queue APIC reschedules during early boot.
    system.cpu.eventq_index = 0
...
# Minimal-difference change from fpga_fs_complete.py:
# switch the SE MMIO device to the PCIe/TLM-oriented FPGAAccelerator.
...
pci_dev8_inta = X86IntelMPIOIntAssignment(
    interrupt_type='INT',
    polarity='ConformPolarity',
    trigger='ConformTrigger',
    source_bus_id=0,
    source_bus_irq=0 + (8 << 2),
    dest_io_apic_id=io_apic.id,
    dest_io_apic_intin=16,
)
base_entries.append(pci_dev8_inta)
madt_records.append(
    X86ACPIMadtIntSourceOverride(
        bus_source=pci_dev8_inta.source_bus_id,
        irq_source=pci_dev8_inta.source_bus_irq,
        sys_int=pci_dev8_inta.dest_io_apic_intin,
        flags=0,
    )
)
...
system.workload.command_line = os.environ.get(
    'GEM5_KERNEL_CMDLINE',
    'earlyprintk=ttyS0 console=ttyS0 ignore_loglevel '
    'root=/dev/ram0 rdinit=/init_pci fpga_mode=pci'
)
