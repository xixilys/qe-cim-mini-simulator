...
constexpr uint64_t COMMAND_WINDOW_ELECTRONS_CMD = 0x1000;
constexpr uint64_t DMA_DESC_WINDOW_SRC_LO = 0x2000;
constexpr uint64_t RESULT_MAILBOX_CONVERGED = 0x3000;
...
std::string backend_class_for_mode(const std::string& mode) {
  if (mode == "systemc_standalone") {
    return "systemc_standalone_proxy";
  }
  return "systemc_timed_functional_proxy";
}

std::string source_kind_for_mode(const std::string& mode) {
  (void)mode;
  return "backend_runner_systemc";
}
...
SC_MODULE(TestBench) {
  ...
  void run_tlm_address_map_tests() {
    ...
    tlm_write32(*tlm_target, COMMAND_WINDOW_ELECTRONS_CMD, 1, response);
    passed &= expect_true(response == TLM_OK_RESPONSE,
                          "canonical command queue window routes to electrons command");
    uint32_t canonical_command_count =
        tlm_read32(*tlm_target, COMMAND_WINDOW_ELECTRONS_CMD, response);
    passed &= expect_true(response == TLM_OK_RESPONSE &&
                              canonical_command_count == command_count + 1,
                          "canonical command queue readback aliases command counter only");
    tlm_write32(*tlm_target, DMA_DESC_WINDOW_SRC_LO, static_cast<uint32_t>(src_addr), response);
    passed &= expect_true(response == TLM_OK_RESPONSE,
                          "canonical DMA descriptor window routes to DMA SRC_LO");
    uint32_t canonical_src_lo = tlm_read32(*tlm_target, DMA_DESC_WINDOW_SRC_LO, response);
    passed &= expect_true(response == TLM_OK_RESPONSE &&
                              canonical_src_lo == static_cast<uint32_t>(src_addr),
                          "canonical DMA descriptor readback is consistent");
    uint32_t canonical_converged =
        tlm_read32(*tlm_target, RESULT_MAILBOX_CONVERGED, response);
    passed &= expect_true(response == TLM_OK_RESPONSE &&
                              (canonical_converged == 0 || canonical_converged == 1),
                          "canonical result mailbox aliases converged status");
  ...
  bool write_backend_execution_report() const {
    ...
    out << "  \"backend_class\": " << json_string(backend_class_for_mode(mode)) << ",\n";
    out << "  \"source_kind\": " << json_string(source_kind_for_mode(mode)) << ",\n";
    out << "  \"environment\": {\n";
    ...
    out << "    \"mmio_read_count\": " << tlm_target->mmio_read_count() << ",\n";
    out << "    \"mmio_write_count\": " << tlm_target->mmio_write_count() << ",\n";
    ...
    out << "    \"host_control_mmio_read_count\": " << tlm_target->mmio_read_count() << ",\n";
    out << "    \"host_control_mmio_write_count\": " << tlm_target->mmio_write_count() << ",\n";
    out << "    \"host_control_polling_read_count\": " << tlm_target->polling_read_count() << ",\n";
    out << "    \"host_control_interrupt_count\": " << tlm_target->interrupt_count() << ",\n";
    out << "    \"systemc_datapath_device_busy_ns\": "
        << (has_electrons_result ? last_electrons_result.device_busy_ns : 0) << ",\n";
    out << "    \"systemc_datapath_dma_read_bytes\": "
        << (has_electrons_result ? last_electrons_result.dma_read_bytes : 0) << ",\n";
    out << "    \"systemc_datapath_dma_write_bytes\": "
        << (has_electrons_result ? last_electrons_result.dma_write_bytes : 0) << ",\n";
