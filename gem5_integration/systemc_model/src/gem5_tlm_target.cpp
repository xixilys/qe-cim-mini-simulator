      ...
      last_control_policy_("sync")
{
  ...
  std::cout << "[" << sc_time_stamp() << "] " << name
            ...
            << "control=0x0000-0x0fff, command=0x1000-0x1fff, "
            << "dma_desc=0x2000-0x2fff, result_mailbox=0x3000-0x3fff, "
            << "legacy_register_aliases=0x0000-0x013f, hbm=0x" << std::hex
...
void Gem5TLMTarget::handle_read(tlm_generic_payload& trans, sc_time& delay) {
  uint64_t addr = normalize_register_address(trans.get_address());
  unsigned char* data_ptr = trans.get_data_ptr();
  unsigned int data_length = trans.get_data_length();
...
bool Gem5TLMTarget::is_legacy_register_address(uint64_t addr) const {
  switch (addr) {
    case REG_CONTROL:
    case REG_STATUS:
    case REG_INTERRUPT:
...
bool Gem5TLMTarget::is_canonical_register_address(uint64_t addr) const {
  switch (addr) {
    case COMMAND_ELECTRONS_CMD:
    case DMA_DESC_SRC_LO:
    case DMA_DESC_SRC_HI:
    case DMA_DESC_DST_LO:
    case DMA_DESC_DST_HI:
    case DMA_DESC_SIZE:
    case DMA_DESC_CONTROL:
    case RESULT_MAILBOX_ELECTRONS_CONVERGED:
      return true;
    default:
      return false;
  }
...
bool Gem5TLMTarget::is_register_address(uint64_t addr) const {
  return is_legacy_register_address(addr) || is_canonical_register_address(addr);
}

uint64_t Gem5TLMTarget::normalize_register_address(uint64_t addr) const {
  switch (addr) {
    case COMMAND_ELECTRONS_CMD:
      return REG_ELECTRONS_CMD;
    case DMA_DESC_SRC_LO:
      return REG_DMA_SRC_LO;
    case DMA_DESC_SRC_HI:
      return REG_DMA_SRC_HI;
    case DMA_DESC_DST_LO:
      return REG_DMA_DST_LO;
    case DMA_DESC_DST_HI:
      return REG_DMA_DST_HI;
    case DMA_DESC_SIZE:
      return REG_DMA_SIZE;
    case DMA_DESC_CONTROL:
      return REG_DMA_CONTROL;
    case RESULT_MAILBOX_ELECTRONS_CONVERGED:
      return REG_ELECTRONS_CONVERGED;
    default:
      return addr;
  }
}
